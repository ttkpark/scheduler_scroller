 ...
 *
 * 환경변수:
 *   DISABLE_EASYOCR=1  → EasyOCR를 건너뛰고 GPT-4o / Claude / Tesseract 순으로만 동작
 *   OCR_ENGINE=tesseract → 위와 동일 (EasyOCR 비활성)
 *   EASYOCR_TIMEOUT_MS   → EasyOCR 응답 대기 시간(ms), 기본 180000
 *   OPENAI_API_KEY       → GPT-4o Vision (EasyOCR 실패·비활성 시)
...
/** 원격 서버 등에서 EasyOCR(torch)를 쓰지 않을 때 true */
function isEasyOCRDisabled() {
  const d = (process.env.DISABLE_EASYOCR || '').toLowerCase().trim();
  if (d === '1' || d === 'true' || d === 'yes') return true;
  const engine = (process.env.OCR_ENGINE || '').toLowerCase().trim();
  return engine === 'tesseract' || engine === 'tesseract-only';
}
...
async function recognize(imagePath) {
  // 1순위: EasyOCR (DISABLE_EASYOCR=1 또는 OCR_ENGINE=tesseract 이면 생략)
  if (!isEasyOCRDisabled()) {
    try {
    ...
    console.log('[OCR] EasyOCR 비활성화됨 (DISABLE_EASYOCR / OCR_ENGINE=tesseract)');
  }
